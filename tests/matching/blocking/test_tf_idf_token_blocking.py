...
def test_abt_buy_blocking(abt, buy, abt_buy_identifier):
    engine = BlockEngine().add_source(
        abt, abt_buy_identifier
    ).add_source(
        buy, abt_buy_identifier
    ).tf_idf(1)

    assert engine.blocks


def test_abt_buy_tf_idf_blocking_metrics(abt, buy, abt_buy_identifier, abt_buy_perfect_mapping):
    engine = BlockEngine().add_source(
        abt, abt_buy_identifier
    ).add_source(
        buy, abt_buy_identifier
