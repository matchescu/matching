...
def test_fs_matcher(table_a, table_b, ground_truth):
    rl_config = RecordLinkageConfig(
        "idA", "idB", "label", [("fname", "fname"), ("lname", "lname")]
    )
...
def test_empirical_error_rates_on_train_holdout(
    table_a, table_b, ground_truth, id_pairs
):
    rl_config = RecordLinkageConfig(
        "idA", "idB", "label", [("fname", "fname"), ("lname", "lname")]
    )
    fs = FellegiSunter(rl_config, mu=0.25, lambda_=0.25).fit(
