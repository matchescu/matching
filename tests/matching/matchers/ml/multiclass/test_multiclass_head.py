...
def test_apply_head_when_head_type_none_returns_two_chunk_width(make_params):
    ...
    torch.testing.assert_close(
        result, torch.cat([enc_a, enc_b], dim=-1), rtol=0, atol=0
    )
...
def test_apply_head_when_head_type_abs_returns_three_chunk_width(make_params):
    ...
    torch.testing.assert_close(
        result, torch.cat([enc_a, enc_b, (enc_a - enc_b).abs()], dim=-1), rtol=0, atol=0
    )
...
@pytest.mark.parametrize("head", [HeadType.NONE, HeadType.ABS])
def test_existing_head_preserves_classifier_state_keys(make_params, head):
    module = MultiClassModule(make_params(head_type=head))
    assert set(module.classifier.state_dict()) == {
        "_model.0.weight",
        "_model.0.bias",
        "_model.3.weight",
        "_model.3.bias",
