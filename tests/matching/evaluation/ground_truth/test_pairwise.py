...
@pytest.mark.parametrize(
    "mapping_file_path", ["amazon_google_exp_data/test.csv"], indirect=True
)
def test_read_csv_with_label(mapping_file_path):
    ...
    assert len(gt) != 2293
    assert len(gt) == 234
    assert all(x.source == "amz-ggl" and y.source == "amz-ggl" for x, y in gt)


@pytest.mark.parametrize(
    "mapping_file_path,expected",
    [
...
def test_read_csv_must_specify_sources(mapping_file_path):
    ...
    assert (
        str(err_proxy.value)
        == "at least one of 'sources' or 'source_cols' must be specified"
    )
...
def test_read_csv_must_specify_sources_unambiguously(mapping_file_path):
    ...
    assert (
        str(err_proxy.value)
        == "only one of 'sources' and 'source_cols' can be specified"
    )
...
@pytest.mark.parametrize(
    "mapping_file_path", ["abt-buy/abt_buy_perfectMapping.csv"], indirect=True
)
