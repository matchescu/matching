import logging
from pathlib import Path

from torch.utils.data import DataLoader
from sklearn.metrics import f1_score

from matchescu.matching.matchers.ml.training import BaseEvaluator

from .._module import DeepERModule
from ._config import CAPABILITY
from ._dataset import DeepERDataset


class DeepEREvaluator(
    BaseEvaluator[DeepERModule, DeepERDataset], capability=CAPABILITY
):
    def __init__(
        self,
        task_name: str,
        dev_data: DataLoader[DeepERDataset],
        test_data: DataLoader[DeepERDataset],
        tb_log_dir: Path,
        logger: logging.Logger | None = None,
    ) -> None:
        super().__init__(task_name, dev_data, test_data, tb_log_dir, logger)
        self._best_f1 = 0.0

    def _run_model(
        self,
        model: DeepERModule,
        data: DataLoader[DeepERDataset],
        best_config: dict | None = None,
    ) -> tuple[bool, dict]:
        y_pred = []
        y_true = []

        for left_attrs, right_attrs, labels in data:
            logits = model(left_attrs, right_attrs)
            y_pred.extend(logits.argmax(dim=1).detach().cpu().numpy())
            y_true.extend(labels.detach().cpu().numpy())

        f1 = f1_score(y_true, y_pred)

        if self._is_evaluating(best_config):
            best_config.update({"test_f1": f1})
            return True, best_config
        else:
            if f1 > self._best_f1:
                self._best_f1 = f1
                return True, {"dev_f1": f1}
            else:
                return False, {}
